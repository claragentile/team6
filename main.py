...
""""" until the P = f.generate_patterns line 41
...
for i in range(np.size(patterns,0)):
    ...
    historic_pertubed_p = f.dynamics_async(perturbed_pattern, f.hebbian_weights(patterns), 5000,1500)
...
for i in range(np.size(patterns,0)):
    perturbed_pattern = f.perturb_pattern(patterns[i], 200)
    historic_pertubed_p = f.dynamics_async(perturbed_pattern, f.storkey_weights(patterns), 5000,1500)
    if (historic_pertubed_p[-1] != patterns[i]).all():
        answer_2 += 1
